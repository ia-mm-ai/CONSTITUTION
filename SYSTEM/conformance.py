"""Derivative execution of STATE's formal notation; not constitutional qualification."""
...
from jsonschema import Draft202012Validator, FormatChecker, RefResolver, ValidationError, validators
...
class Core:
    def __init__(self, root=ROOT):
        ...
        for path in sorted((self.root / "STATE").glob("*/SCHEMAS/*.schema.json")):
            ...
            if not identifier or identifier in self.schemas:
                raise ValueError(f"Missing or duplicate schema ID: {path}")
    ...
    def check_schema(self, identifier):
        ...
        resolver = self.validator(identifier).resolver
    ...
    def check_records(self):
        ...
        for path in records:
            ...
            for relative in record["target_files"]:
                ...
                if target.name in ("CONTINUITY-STATE-CAPABILITY.json", "DYNAMIC-CAPACITY-REGULATION.json"):
        ...
        for path in sorted((self.root / "STATE/LINEAGE/IMPLEMENTATIONS").glob("*.json")):
            ...
            result = semantic_result(model["rules"], claim)
            if result["failure_codes"]:
                raise ValueError(f"{path.name}: {result['failure_codes']}")
