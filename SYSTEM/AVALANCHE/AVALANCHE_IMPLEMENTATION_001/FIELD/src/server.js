...
// The FIELD loopback API exposes only public, accepted-state views and strict
// authorization requests. Reaching it is API access, never presence: a caller
// that can read /health has not thereby entered anything.
export function createFieldServer(runtime) {
  return createServer(async (request, response) => {
    try {
      ...
      if (request.method === "GET" && segments.length === 1 && segments[0] === "health") {
        ...
        send(response, 200, { healthy: true, access_is_not_presence: true, ...observationView(observed) });
        return;
      }
...
export async function listen(runtime) {
  const server = createFieldServer(runtime);
