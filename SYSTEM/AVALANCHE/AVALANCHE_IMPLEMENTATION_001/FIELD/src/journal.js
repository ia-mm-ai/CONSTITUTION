...
import { FIELD_EVENT_SCHEMA } from "./constants.js";

const ZERO = "0".repeat(64);
const RESERVED = new Set(["schema", "sequence", "previous_event_sha256", "recorded_at", "type", "event_sha256"]);
const FORBIDDEN_CONTENT_FIELDS = new Set(["payload", "unsigned", "signature", "private_key", "private_state", "content", "secret", "seed"]);
...
// AppendOnlyJournal is the FIELD's local evidence ledger: a permissioned,
// append-only SHA-256 hash chain of decisions and public commitments. It stores
// no private material and never overwrites a prior record, so corrections and
// departures are additive and every earlier entry stays readable.
export class AppendOnlyJournal {
  ...
  async initialize() {
    ...
    try {
      ...
      for (const line of contents.split("\n")) {
        ...
        if (event.schema !== FIELD_EVENT_SCHEMA || event.sequence !== expectedSequence || event.previous_event_sha256 !== previous) {
  ...
  // readAll returns the committed chain as parsed events, newest last. It is a
  // read of local evidence only; it never asserts current presence.
  async readAll() {
    try {
      const contents = await readFile(this.path, "utf8");
