...
import { capacityFromAcceptedState } from "./noncollapse.js";
...
export function validateVMState(state) {
  ...
  if (state.profile_form_id !== VM_FORM_ID || state.profile_form_sha256 !== VM_FORM_SHA256) {
    throw new Error("VM form identity does not match the bound PRESENCE_AVALANCHE_VM_001 form");
  }
...
// capacityView keeps the two capacities strictly separate. Structural capacity
// is the fixed ceiling the genesis set; situated capacity is what is actually
// available now (declared_actual - unresolved - correction_egress). They are
// never added, averaged, or substituted for one another. A token balance is not
// admitted as either (refused inside capacityFromAcceptedState).
export function capacityView(snapshot) {
  assertPlainObject(snapshot, "capacity snapshot");
  capacityFromAcceptedState(snapshot);
  for (const field of [
    "structural_ceiling_units", "declared_actual_units", "effective_actual_units",
    "unresolved_units", "correction_egress_units", "available_units", "deficit_units"
  ]) {
    if (Object.hasOwn(snapshot, field)) requireSafeUint(snapshot[field], `capacity.${field}`);
  }
  return {
    structural_ceiling_units: snapshot.structural_ceiling_units ?? 0,
    situated_available_units: snapshot.available_units ?? 0,
    declared_actual_units: snapshot.declared_actual_units ?? 0,
    effective_actual_units: snapshot.effective_actual_units ?? 0,
    unresolved_units: snapshot.unresolved_units ?? 0,
    correction_egress_units: snapshot.correction_egress_units ?? 0,
    deficit_units: snapshot.deficit_units ?? 0,
    capacity_rule: "STRUCTURAL_IS_FIXED_CEILING_SITUATED_IS_AVAILABLE_NEVER_CONFLATED"
  };
}
...
// makeObservation records what the FIELD reads on a controlled surface. It binds
// the observation to an adapter_id + controlled_surface, never to a kind of
// being, and stamps SURFACE_ONLY_NOT_PARTICIPANT_TYPE. It exposes structural and
// situated capacity explicitly and separately.
export function makeObservation(status, state, actorId, adapter, observedAt = Math.floor(Date.now() / 1000)) {
  ...
  return {
    ...
    controlled_surface: adapter.controlled_surface,
    attribution: SURFACE_ATTRIBUTION,
    phase: actor.phase,
    ...
    capacity: capacityView(status.capacity),
    vm_height: status.height,
    effect: "OBSERVES_ACCEPTED_LOCALITY_STATE_ON_CONTROLLED_SURFACE_ONLY",
    non_effect: "DOES_NOT_ESTABLISH_EXTERNAL_BODY_OBEDIENCE_OR_PARTICIPANT_TYPE"
  };
}
