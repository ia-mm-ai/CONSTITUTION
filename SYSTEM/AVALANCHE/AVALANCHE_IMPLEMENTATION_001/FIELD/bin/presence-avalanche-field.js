...
import { FIELD_PROTOCOL, FIELD_VERSION, IMPLEMENTATION_ID, IMPLEMENTATION_VERSION, VM_ID, VM_VERSION } from "../src/constants.js";
...
async function main() {
  ...
  if (args.length === 1 && args[0] === "--version-json") {
    process.stdout.write(`${JSON.stringify({
      ...
      implementation: `${IMPLEMENTATION_ID}/${IMPLEMENTATION_VERSION}`,
      vm_id: VM_ID,
  ...
  if (command === "serve") {
    ...
    process.stdout.write(`${JSON.stringify({ status: "LISTENING", host: address.address, port: address.port, field_id: runtime.config.field_id })}\n`);
