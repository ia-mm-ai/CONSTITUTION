...
var (
	...
	errNoPendingTransitions = errors.New("no valid pending PRESENCE AVALANCHE transitions")
	errInvalidPreference    = errors.New("preferred block is unavailable or unverified")
)

var (
	genesisBytesKey = []byte("presence/meta/genesis-bytes")
	genesisIDKey    = []byte("presence/meta/genesis-id")
	lastAcceptedKey = []byte("presence/meta/last-accepted")
	currentStateKey = []byte("presence/state/current")
)
...
type VM struct {
	...
	genesis        *Genesis
	genesisBytes   []byte
	genesisBlock   *presenceBlock
	blocks         map[ids.ID]*presenceBlock
...
func (vm *VM) Initialize(
	...
	vm.blocks = map[ids.ID]*presenceBlock{genesisBlock.id: genesisBlock}
...
func (vm *VM) verifyBlock(block *presenceBlock) error {
	vm.lock.Lock()
	defer vm.lock.Unlock()
	if !vm.initialized {
		return errNotInitialized
...
func (vm *VM) acceptBlock(block *presenceBlock) error {
	vm.lock.Lock()
	defer vm.lock.Unlock()
	if !vm.initialized {
		return errNotInitialized
	...
	// Durable writes are staged first and the in-memory acceptance state is
	// mutated only after the commit succeeds. A failed commit must leave pending
	// and accepted state exactly as it was.
	acceptedTransitionIDs := make([]string, 0, len(block.wire.Transitions))
	for i, transition := range block.wire.Transitions {
		...
		acceptedTransitionIDs = append(acceptedTransitionIDs, transitionIDHex)
	}
	if err := vdb.Commit(); err != nil {
		return fmt.Errorf("commit accepted PRESENCE AVALANCHE block: %w", err)
	}
	for _, transitionIDHex := range acceptedTransitionIDs {
...
func (vm *VM) rejectBlock(block *presenceBlock) error {
	vm.lock.Lock()
	defer vm.lock.Unlock()
	if !vm.initialized {
		return errNotInitialized
...
func (vm *VM) getBlockLocked(blockID ids.ID) (*presenceBlock, error) {
...
func (vm *VM) loadAcceptedBlockLocked(blockID ids.ID) (*presenceBlock, error) {
...
func blockKey(blockID ids.ID) []byte {
	return append([]byte("presence/block/"), blockID[:]...)
}

func stateByBlockKey(blockID ids.ID) []byte {
	return append([]byte("presence/state/block/"), blockID[:]...)
}

func heightKey(height uint64) []byte {
	key := make([]byte, len("presence/height/")+8)
	copy(key, "presence/height/")
	binary.BigEndian.PutUint64(key[len("presence/height/"):], height)
	return key
}

func transitionKey(transitionID string) []byte { return []byte("presence/transition/" + transitionID) }
func receiptKey(transitionID string) []byte    { return []byte("presence/receipt/" + transitionID) }
