	...
	genesis := &Genesis{
		...
		Constitution: GenesisConstitution{
			...
			PublicOrigin: GenesisConstitutionOrigin{
				...
				HumanBytes: 44133, MachineBytes: 146048,
				Binding:           "SOURCE_AT_STARTING_COMMIT_WITH_SUCCESSOR_LOCAL_STATE_BINDING",
				MirrorEffect:      "NO_BUNDLED_SOURCE_MIRROR",
				PublicationEffect: "PUBLICATION_NOT_FORMATION",
			},
		...
		Capacity: GenesisCapacity{
			Unit: "PRESENCE_AVALANCHE_CAPACITY_UNIT", StructuralCeilingUnits: 100,
...
func mustApply(t *testing.T, state *RuntimeState, genesis *Genesis, transition *Transition) (*RuntimeState, *Receipt, string) {
	...
	contract, classified := operationContracts[transition.Unsigned.Operation]
	if !classified || receipt.Operation != contract.Operation || receipt.Effect != contract.Effect ||
		next.Events[receipt.TransitionID].Effect != contract.Effect {
		t.Fatalf("accepted %s receipt/event diverges from the normative effect inventory", transition.Unsigned.Operation)
	}
