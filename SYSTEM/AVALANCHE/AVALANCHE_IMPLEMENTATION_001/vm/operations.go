...
const (
	residueSchema           = "PRESENCE_AVALANCHE_ADDRESSED_RESIDUE_001"
	residueEffect           = "AVAILABLE_FOR_ADDRESSEE_DECISION_ONLY"
	residueIDDomain         = "PRESENCE_AVALANCHE_RESIDUE_ID_001"
	continuityStepSchema    = "PRESENCE_AVALANCHE_PARTICIPANT_STATE_SUCCESSION_001"
	continuityPassageSchema = "PRESENCE_AVALANCHE_PARTICIPANT_STATE_PASSAGE_001"
)
...
func (s *RuntimeState) applyOperation(genesis *Genesis, transition *Transition, transitionID string) error {
	...
	if err := s.validateOperationScope(u); err != nil {
		return err
	}
	switch u.Operation {
	case opBound:
...
func (s *RuntimeState) currentnessCommitment(observedAt int64, carrierSetSHA256 string) (string, error) {
	...
	envelope := currentnessEnvelope{
		Schema: "PRESENCE_AVALANCHE_CURRENTNESS_001", PreviousStateCommitment: s.StateCommitment,
...
func successorProposalCommitment(p *ProposeSuccessorPayload) (string, error) {
	...
	domainBytes := append([]byte("PRESENCE_AVALANCHE_SUCCESSOR_PROPOSAL_001\x00"), bytes...)
