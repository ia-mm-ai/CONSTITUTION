...
if [[ ! -x "${plugin_path}" ]]; then
  echo "PRESENCE AVALANCHE VM plugin is missing or not executable: ${plugin_path}" >&2
  exit 1
fi
...
if [[ "${plugin_version}" != *"avalanchego-profile=v1.15.0+PRESENCE_SECURITY_OVERLAY_001"* || "${plugin_version}" != *"rpcchainvm-protocol=46"* ]]; then
  echo "unexpected PRESENCE AVALANCHE VM plugin compatibility declaration: ${plugin_version}" >&2
  exit 1
fi
